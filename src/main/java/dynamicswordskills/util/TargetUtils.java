...
public class TargetUtils
{
	...
	public static double getReachDistanceSq(EntityPlayer player) {
		/*
		 * if (DynamicSwordSkills.isBG2Enabled) { float reach =
		 * getBattlegearReachDistance(player); return reach * reach; }
		 */
	...
	private static float getBattlegearReachDistance(EntityPlayer player) {
		...
		if (mainhand == null) {
			 ...
			 * else if (mainhand.getItem() instanceof IExtendedReachWeapon) { reachMod =
			 * ((IExtendedReachWeapon)
			 * mainhand.getItem()).getReachModifierInBlocks(mainhand); }
			 */
		return reachMod;
	}
	...
	public static void applyExtendedReachModifier(EntityPlayer player, double range) {
		return;
		/*
		 * if (!DynamicSwordSkills.isBG2Enabled) { return; } double reach =
		 * Math.sqrt(getReachDistanceSq(player)); if (reach < range) {
		 * TargetUtils.removeExtendedReachModifier(player); IAttributeInstance instance
		 * = player.getEntityAttribute(Attributes.extendedReach); if (instance != null)
		 * { double mod = range - reach; AttributeModifier modifier = (new
		 * AttributeModifier(reachModifierID, "Skill Reach Modifier", mod,
		 * 0)).setSaved(false); instance.applyModifier(modifier); } }
		 */
	}
	...
	public static void removeExtendedReachModifier(EntityPlayer player) {
		return;
		/*
		 * if (!DynamicSwordSkills.isBG2Enabled) { return; } IAttributeInstance instance
		 * = player.getEntityAttribute(Attributes.extendedReach); if (instance != null)
		 * { AttributeModifier prev = instance.getModifier(reachModifierID); if (prev !=
		 * null) { instance.removeModifier(prev); } }
		 */
	}
